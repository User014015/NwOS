...
echo [4/10] Compiling random word...
clang --target=i386-pc-none-elf -m32 -ffreestanding -fno-stack-protector -fno-pic -c sys/kernel/Apps/wordgenerator/wordgenerator.c
if errorlevel 1 goto error

echo [5/10] Compiling keyboard driver...
clang --target=i386-pc-none-elf -m32 -ffreestanding -fno-stack-protector -fno-pic -c drivers/keyboard.c -o keyboard.o
if errorlevel 1 goto error

echo [6/10] Compiling disk driver...
clang --target=i386-pc-none-elf -m32 -ffreestanding -fno-stack-protector -fno-pic -c drivers/disk.c -o disk.o
if errorlevel 1 goto error

echo [7/10] Compiling kernel panic...
...
echo [8/10] Linking...
ld.lld -m elf_i386 -T linker.ld kernel_entry.o kernel.o keyboard.o disk.o kernelpanic.o wordgenerator.o -o kernel.elf
if errorlevel 1 goto error

echo [9/10] Creating kernel binary...
llvm-objcopy -O binary kernel.elf kernel.bin
if errorlevel 1 goto error

echo [10/10] Creating NwOS.img...
